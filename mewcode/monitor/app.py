"""Standalone terminal monitor for MewCode provider request history."""
...
def _workspace_records(workspace: str) -> list[dict[str, Any]]:
    root = Path(workspace) / ".mewcode" / "sessions"
...
class MonitorApp:
    ...
    def _left_content(self):
        ...
        if not self.groups:
            fragments.append(("", "等待 MewCode 请求..."))
            return fragments
        for index, group in enumerate(self.groups):
            record = group[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="mewcode-monitor",
        description="Monitor MewCode requests sent to LLM providers.",
    )
    parser.add_argument(
        "-w",
        "--workspace",
        default=os.getcwd(),
        help="workspace containing .mewcode/sessions",
    )
