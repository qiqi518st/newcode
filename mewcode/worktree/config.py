...
与 agents 配置同目录族：`.mewcode/config.local.yaml` / `.mewcode/config.yaml` /
`~/.mewcode/config.yaml`，`worktrees:` 键局部优先（镜像 subagent/config.py 模式）。
"""
...
_WORKTREES_FILE_LOCAL = ".mewcode/config.local.yaml"
_WORKTREES_FILE_PROJECT = ".mewcode/config.yaml"
_WORKTREES_FILE_USER = "~/.mewcode/config.yaml"
