...
均降级为跳过该层或该 server 并 stderr 告警，决不阻断 MewCode 启动。

- 用户级：<home>/.mewcode/config.yaml
...
def _load_file(path: Path) -> dict[str, _RawServer]:
    ...
    防回归：曾出现 YAML 格式错误直接抛 YAMLError 致 mewcode 启动失败（spec F1/N1 要求绝不致启动失败）。
    """
    if not path.exists():
        return {}
    try:
...
def load_mcp_servers(root: str) -> dict[str, ServerConfig]:
    ...
    """
    # 用户级：<home>/.mewcode/config.yaml（Path.home 抛错时跳过用户层）
    try:
    ...
    project_path = Path(root) / ".mewcode.yaml"
