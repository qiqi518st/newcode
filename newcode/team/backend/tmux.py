"""tmux 后端（ch15 F3/F15/F16）：独立窗格跑完整 `newcode --team-member` 实例。
...
- 命令构造：`python -m newcode --team-member ...`，**必带预生成的 `--agent-id`**
...
def build_member_cmd(req: SpawnRequest) -> list[str]:
    """构造 `newcode --team-member` 命令行（含预生成 --agent-id，F3.2/F15）。"""
    parts = [
        sys.executable,
        "-m",
        "newcode",
...
class TmuxBackend:
    ...
    async def spawn(self, req: SpawnRequest) -> tuple[str, str]:
        ...
        if not os.environ.get("TMUX"):
            ...
            session = f"newcode-team-{req.team_name}-{req.member_name}"
