"""Safe, cached loading of layered NEWCODE.md instruction files."""
...
class InstructionLoader:
    ...
    def load(self, *, refresh: bool = False) -> InstructionDocument:
        ...
        total = 0
        candidates = [
            self.project_root / "NEWCODE.md",
            self.project_root / ".newcode" / "NEWCODE.md",
            self.user_home / ".newcode" / "NEWCODE.md",
        ]
    ...
    def _allowed_root(self, source: Path) -> Path:
        return (
            self.user_home / ".newcode"
            if source.is_relative_to(self.user_home / ".newcode")
