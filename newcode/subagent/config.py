...
与 hooks 配置同一目录族：`.newcode/config.local.yaml` / `.newcode/config.yaml` /
`~/.newcode/config.yaml`，`agents:` 键局部优先（spec F11.1）。
"""
...
_AGENTS_FILE_LOCAL = ".newcode/config.local.yaml"
_AGENTS_FILE_PROJECT = ".newcode/config.yaml"
_AGENTS_FILE_USER = "~/.newcode/config.yaml"
