...
加载顺序（高优先级先写，后扫到的同名只写入 key 尚未存在的——前者覆盖后者）：
  项目级 <root>/.newcode/agents → 用户级 ~/.newcode/agents → 内置
  newcode/subagent/builtin（importlib.resources）→ 插件级（本期跳过，占位）。
...
_BUILTIN_PKG = "newcode.subagent.builtin"
...
def load_catalog(project_root: str, agents_cfg: AgentConfig | None = None) -> Catalog:
    ...
    _load_dir(catalog, Path(project_root) / ".newcode" / "agents", Source.PROJECT)
    _load_dir(catalog, Path.home() / ".newcode" / "agents", Source.USER)
