...
- 三级搜索路径（F2.1）：内置 <newcode/skills/builtin/> → 用户 ~/.newcode/skills/ →
  项目 <work_dir>/.newcode/skills/，后扫同名覆盖前者（项目级 > 用户级 > 内置级）。
...
- disabled 集合读写 ~/.newcode/skills/disabled.json（F7.8 跨会话持久）。
"""
