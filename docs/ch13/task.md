# MewCode ch13 - 多 Agent 分发架构 Tasks
...
>
> 版本管理：本章开发前先升 0.13.0（T1），`mewcode/__init__.py` 与 `pyproject.toml` 同步。
>
...
| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `mewcode/__init__.py`、`pyproject.toml` | 版本号 0.12.0 → 0.13.0 |
| 新建 | `mewcode/subagent/__init__.py` | 包导出（AgentDefinition/Source/Catalog/TaskManager/...） |
| 新建 | `mewcode/subagent/types.py` | AgentDefinition / Source / DefinitionParseError / 文案常量 |
| 新建 | `mewcode/subagent/config.py` | AgentConfig + load_agent_config（agents: 段三层合并）+ effective_ 方法 |
| 新建 | `mewcode/subagent/parser.py` | parse_definition（复用 skills frontmatter 分离） |
| 新建 | `mewcode/subagent/catalog.py` | Catalog 四层加载 + 优先级 + resolve/list + fork_definition |
| 新建 | `mewcode/subagent/builtin/{general-purpose,explore,plan,verifier}.md` | 4 个内置角色 |
| 新建 | `mewcode/subagent/fork.py` | build_forked_messages + FORK_BOILERPLATE + is_fork_context |
| 新建 | `mewcode/subagent/launcher.py` | SubAgentLauncher（子 Registry/模型/子 Agent/前台后台分派） |
| 新建 | `mewcode/subagent/manager.py` | BackgroundTask + TaskManager（launch/foreground/adopt/continue 同 id/清理/done） |
| 新建 | `mewcode/subagent/errors.py` | MaxTurnsReached |
| 新建 | `mewcode/tools/filter.py` | GLOBAL_DENY / ASYNC_AGENT_ALLOWED_TOOLS / apply_agent_tool_filter |
| 新建 | `mewcode/tools/agent_tool.py` | AgentTool（description 渲染角色列表 + execute 分派） |
| 新建 | `mewcode/tools/task_tools.py` | TaskListTool / TaskGetTool / TaskStopTool / SendMessageTool |
| 修改 | `mewcode/tools/registry.py` | Registry.view(visible) |
| 修改 | `mewcode/agent/agent.py` | max_turns/dont_ask 参数、_react 抽取、run_to_completion、dont_ask 短路 |
| 修改 | `mewcode/permission/checker.py` | for_subagent()（共享规则层） |
| 修改 | `mewcode/hooks/executor.py` | _run_agent 真实实现（launcher 回调） |
| 修改 | `mewcode/hooks/engine.py` | set_agent_launcher |
| 新建 | `mewcode/slash/commands/tasks.py` | /tasks 命令族 |
| 修改 | `mewcode/slash/context.py` | task_manager 字段 |
| 修改 | `mewcode/slash/commands/__init__.py` | register_all 注册 /tasks |
| 修改 | `mewcode/tui/app.py` | done 队列消费、clear_all、foreground_sub_agent 预留字段 |
| 修改 | `mewcode/skills/executor.py` | _execute_fork 走 launcher（F10 底座统一） |
| 修改 | `mewcode/main.py` | 装配（config/catalog/manager/launcher/tools/hooks/tui） |
...
**依赖：** 无
**完成标准：**
- [ ] `mewcode/__init__.py` 的 `__version__` = `"0.13.0"`
- [ ] `pyproject.toml` 的 `version` = `"0.13.0"`，两处一致
- [ ] 独立提交 `chore: bump version to 0.13.0`

**验证：** `export PYTHONIOENCODING=utf-8 && python -c "import mewcode; print(mewcode.__version__)"` 输出 0.13.0。

## T2: subagent/types.py —— 数据结构与错误

**文件：** `mewcode/subagent/errors.py`（新建，MaxTurnsReached）、`mewcode/subagent/types.py`（新建）、`mewcode/subagent/__init__.py`（新建）
**依赖：** T1
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent && python -c "from mewcode.subagent.types import AgentDefinition, Source; d=AgentDefinition(name='x',description='d',body='b',source=Source.BUILTIN); assert d.is_fork()==False; assert Source.PROJECT>Source.USER"`。

## T3: subagent/config.py —— agents: 配置段

**文件：** `mewcode/subagent/config.py`（新建）
**依赖：** T2
**完成标准：**
...
- [ ] `load_agent_config(project_root) -> AgentConfig`：读 `.mewcode/config.local.yaml` → `.mewcode/config.yaml` → `~/.mewcode/config.yaml` 的 `agents:` 键（局部优先，缺失全缺省）；`agents:` 键缺失 / 文件不存在 → 全缺省不报错（spec F11.1）
- [ ] 数值字段非法（非 int/float）→ stderr 警告 + 用缺省；`model_tiers` 非 dict → 警告置空

**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/config.py && python -c "import tempfile,os; from mewcode.subagent.config import load_agent_config; ..."`（临时目录断言 enable_verifier=True、model_tiers 命中、缺省兜底、effective_ 返回 True）。

## T4: tools/filter.py + Registry.view

**文件：** `mewcode/tools/filter.py`（新建）、`mewcode/tools/registry.py`（修改）
**依赖：** T1
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/parser.py && python -c "from mewcode.subagent.parser import parse_definition; import tempfile,os; ..."`（临时 .md：合法角色、permissionMode: dontAsk→dont_ask=True、未知 model→inherit、缺 description→抛错）。

## T6: subagent/catalog.py —— 四层加载与优先级

**文件：** `mewcode/subagent/catalog.py`（新建）
**依赖：** T2、T3、T5
**完成标准：**
...
- [ ] `load_catalog(project_root, agents_cfg) -> Catalog`：顺序 ①项目 `<root>/.mewcode/agents/*.md` ②用户 `~/.mewcode/agents/*.md` ③内置 `importlib.resources.files("mewcode.subagent.builtin")` ④插件（跳过）；**高优先级先写**——后扫到的同名只写入 key 尚未存在的（前者覆盖后者，spec F2.3）
- [ ] 内置级解析失败 → raise（N4 fail-fast）；用户/项目级单文件失败 → `print(f"subagent {name}: ... skipped", file=sys.stderr)` 跳过，其余正常加载
- [ ] `agents_cfg.enable_verifier=False` 时跳过内置 verifier.md（spec F2.5）；`.mewcode/agents/` / `~/.mewcode/agents/` 目录不存在 → 跳过不报错
- [ ] `__init__.py` 导出 `Catalog / load_catalog`

**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/catalog.py && python -c "...临时目录：项目级 explore.md 覆盖内置，resolve 返回项目级；用户级独有 custom.md 可 resolve；内置解析坏文件 raise"`。

## T7: subagent/builtin/*.md —— 4 个内置角色

**文件：** `mewcode/subagent/builtin/{general-purpose,explore,plan,verifier}.md`（新建）
**依赖：** T6
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/permission && python -m pytest tests/test_permission_checker.py tests/test_permission_engine.py tests/test_permission_rules.py -q` 全绿；再 `python -c "from mewcode.permission.checker import PermissionChecker; pc=PermissionChecker.create('.'); sub=pc.for_subagent(__import__('mewcode.permission.modes',fromlist=['PermissionMode']).PermissionMode.ACCEPT_EDITS); assert sub.mode is not pc.mode"`。

## T9: agent/agent.py —— max_turns / dont_ask 参数 + _react 抽取

**文件：** `mewcode/agent/agent.py`（修改）
**依赖：** T8
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/agent/agent.py && python -m pytest tests/test_ch12_agent.py -q` 全绿（证明 _react 抽取未破坏 run 行为）。
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/agent mewcode/subagent && python -c "...（mock provider 驱动 run_to_completion：NATURAL 返回文本、max_turns=1 抛 MaxTurnsReached、observer 计数、dont_ask ASK→ALLOW）"`。

## T11: subagent/fork.py —— Fork 消息装填

**文件：** `mewcode/subagent/fork.py`（新建）
**依赖：** T9
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/fork.py && python -c "...（构造带悬空 tool_use 的 parent_conv，断言补全 placeholder、首条消息以 <fork_boilerplate> 起头、is_fork_context True）"`。

## T12: subagent/manager.py —— BackgroundTask + TaskManager

**文件：** `mewcode/subagent/manager.py`（新建）
**依赖：** T10、T11
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/manager.py && python -c "...（mock agent 的 run_to_completion：launch→completed、stop→cancelled、continue_agent 同 id + round 递增 + 上限拒绝、adopt_running 不重跑）"`。

## T13: subagent/launcher.py —— SubAgentLauncher

**文件：** `mewcode/subagent/launcher.py`（新建）
**依赖：** T4、T6、T8、T12
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/subagent/launcher.py && python -c "...（mock catalog/manager/registry，断言 build_sub_registry 过滤、resolve_model 分层、launch_fork 强制后台；前台超时移交后 mock run 计数未归零=未杀重来）"`。

## T14: tools/agent_tool.py —— Agent 工具

**文件：** `mewcode/tools/agent_tool.py`（新建）
**依赖：** T13
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/tools/agent_tool.py && python -c "...（mock launcher/catalog：未知角色 error、description 含角色名、execute 转发参数）"`。

## T15: tools/task_tools.py —— Task 工具组

**文件：** `mewcode/tools/task_tools.py`（新建）
**依赖：** T12
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/tools/task_tools.py && python -c "...（mock manager 断言参数转发与错误路径）"`。

## T16: hooks —— agent 动作接通

**文件：** `mewcode/hooks/executor.py`（修改）、`mewcode/hooks/engine.py`（修改）
**依赖：** T13
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/hooks && python -m pytest tests/test_ch12_executor.py tests/test_ch12_engine.py -q` 全绿；再 mock launcher 断言 agent 动作触发与失败隔离。

## T17: slash —— /tasks 命令族

**文件：** `mewcode/slash/commands/tasks.py`（新建）、`mewcode/slash/context.py`（修改）、`mewcode/slash/commands/__init__.py`（修改）
**依赖：** T12
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/slash && python -c "...（mock manager + mock ui，调 handler 断言列表/详情/终止/续派输出）"`。

## T18: tui —— done 队列消费 + clear_all

**文件：** `mewcode/tui/app.py`（修改）
**依赖：** T12
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/tui && python -m pytest tests/test_ch12_tui.py -q` 全绿（无回归）；mock manager 的 drain_done 断言注入历史与打印（用 `object.__new__` 绕过 PromptSession，按 ch12 手法）。

## T19: skills/executor.py —— fork 底座统一

**文件：** `mewcode/skills/executor.py`（修改）
**依赖：** T13
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/skills && python -m pytest tests/test_ch11_executor.py tests/test_ch11_parser.py -q` 全绿（skill fork 行为不变）。

## T20: main.py 装配

**文件：** `mewcode/main.py`（修改）
**依赖：** T12-T19
**完成标准：**
...
**验证：** `export PYTHONIOENCODING=utf-8 && ruff check mewcode/main.py && python -c "from mewcode.main import main"`（import 无错）；`mewcode --version` 输出 0.13.0。
...
**依赖：** T20
**完成标准：**
- [ ] 定义式前台：`python -m mewcode`（TUI）让主 Agent 调 `agent(subagent_type="explore")` 查一个文件 → 同步返回结果文本
...
**依赖：** T22-T25
**完成标准：**
- [ ] `export PYTHONIOENCODING=utf-8 && ruff format mewcode tests` 后 `ruff check mewcode tests` 全绿（注意：只在 mewcode/ tests/ 下格式化，**禁止** `ruff format .` 以免扫到 docs/）
- [ ] 全量 `python -m pytest tests/ -q` 全绿（含全部存量 ch01-ch12 测试）
- [ ] `git status` 确认 **docs/ 未被任何批量命令改动**（mewcode/、tests/ 之外无异常文件）
- [ ] `python -c "import mewcode; print(mewcode.__version__)"` = 0.13.0
