# NewCode ch15 - AgentTeam 与 Coordinator Mode Checklist
...
- [ ] `team.Manager` 可实例化且构造时 `~/.newcode/teams/` 不存在会自动创建（验证：`python -m newcode` 启动冒烟 + `test_team_manager`）
- [ ] `Manager.create("demo", "")` 在 `~/.newcode/teams/demo/config.json` 落地，`backend` 字段反映 `detect()` 结果（验证：单测 + 检查文件）
- [ ] `Manager.create("foo bar/baz", "")` sanitize 后路径为 `~/.newcode/teams/foo-bar-baz/`（验证：单测）
...
- [ ] `spawn_teammate` 创建 worktree `.newcode/worktrees/team-demo+alice`、`team.members` 含 alice 并持久化到 config.json（验证：单测 + 集成）
...
- [ ] Pane 后端 `python -m newcode --team-member` 子进程**不构造 TUI/REPL**，跑自治协程：读 mailbox → run_to_completion → 通知 Lead idle → stdin 回车做 wake_event 等下一轮（验证：`build_member_cmd` 含 `--agent-id` 单测 + tmux 实跑看 pane 是纯文本日志流）
...
- [ ] `python -m newcode --help` 退出码 0、打印帮助
- [ ] 版本号 0.15.0 在 `newcode/__init__.py` 与 `pyproject.toml` 两处一致（验证：`grep -r "0.15.0"` 两处）
...
- [ ] 可选：`mypy newcode/team/` 全绿

## 端到端场景（tmux 实跑，待人工验证：需真实终端交互）

### 场景 1：tmux 后端，Team 全生命周期

环境：tmux 已装；`cd /path/to/newcode`；开新 tmux 会话 `tmux new-session -s ch15-test`；`python -m newcode` 启动。

- [ ] 输入「创建一个名为 demo 的团队」→ Agent 调 `TeamCreate(team_name="demo")` 返回 `{team_name, backend:"tmux", config_path}`；`ls ~/.newcode/teams/demo/config.json` 存在且 `backend` 为 `tmux`
- [ ] 输入「派 alice 用 general-purpose，在 worktree 里 `echo hello > /tmp/test_alice.txt && pwd > /tmp/test_alice_pwd.txt`」→ Agent 调 `Agent(team_name="demo", name="alice", ...)`
  ...
  - `ls .newcode/worktrees/team-demo+alice/` 存在；30s 内 `/tmp/test_alice.txt` = `hello`、`/tmp/test_alice_pwd.txt` = worktree 路径
...
环境：`.newcode/config.yaml` 加 `features: {coordinator_mode: true}`；`NEWCODE_COORDINATOR_MODE=1` 启动。
...
- [ ] 输入「派几个队员探索 newcode/agent 和 newcode/team」→ Lead 派完队员**不立刻**自己 read_file/glob 探索，回复是「等待汇报中」类措辞；队员 idle 前 Lead 无新工具调用（派完停手纪律验证）

### 场景 4：Plan 审批工作流实跑

- [ ] 准备 `~/.newcode/agents/planner.md`（frontmatter `permission_mode: plan`）；创建 team `plan-test`
...
- [ ] 启动时 `~/.newcode/teams/` 不存在 → 自动创建，不报错
- [ ] `~/.newcode/teams/<name>/config.json` 内容损坏 → 启动只 stderr 警告、跳过该 Team、主流程不崩
