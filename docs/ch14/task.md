# NewCode ch14 - Git Worktree 文件系统隔离 Tasks
...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `newcode/worktree/__init__.py` | 公开导出 Manager / validate_slug / 异常 |
| 新建 | `newcode/worktree/types.py` | Worktree / ExitAction / ExitOptions / ExitReport / AutoCleanupReport / 异常 |
| 新建 | `newcode/worktree/slug.py` | validate_slug / flat_slug / branch_name / is_auto_name / random_agent_name |
| 新建 | `tests/test_worktree_slug.py` | slug 校验单测 |
| 新建 | `newcode/worktree/session.py` | WorktreeSession + JSON 原子持久化 |
| 新建 | `newcode/worktree/git.py` | _run_git + _has_worktree_changes + _resolve_head_sha_from_fs + 高层封装 |
| 新建 | `tests/test_worktree_git.py` | git helper 单测 |
| 新建 | `newcode/worktree/config.py` | WorktreesConfig / load_worktree_config 三层合并 |
| 新建 | `tests/test_worktree_config.py` | 配置缺省/覆盖/降级单测 |
| 新建 | `newcode/worktree/manager.py` | Manager 构造 / 扫描 / list / get / current_session / run 周期 |
| 新建 | `tests/test_worktree_manager.py` | Manager 构造 + session 持久化测试 |
| 新建 | `newcode/worktree/create.py` | create + 快速恢复 + 创建后设置 A/B/C/D |
| 新建 | `tests/test_worktree_create.py` | create + setup 单测 |
| 新建 | `newcode/worktree/lifecycle.py` | enter / exit / remove / auto_cleanup / _has_worktree_changes |
| 新建 | `tests/test_worktree_lifecycle.py` | 生命周期单测 |
| 新建 | `newcode/worktree/sweep.py` | sweep_stale 三层过滤 + EPHEMERAL_PATTERN |
| 新建 | `tests/test_worktree_sweep.py` | sweep_stale 单测 |
| 新建 | `newcode/worktree/notice.py` | build_worktree_notice |
| 新建 | `newcode/tools/cwd.py` | with_cwd / cwd_from_ctx / resolve_path |
| 新建 | `tests/test_tool_ctx.py` | resolve_path 单测 |
| 修改 | `newcode/tools/file_ops.py` | _check_path 沙箱根跟随 ctx cwd（保留沙箱检查） |
| 修改 | `newcode/tools/search.py` | cwd 缺省取 ctx cwd |
| 修改 | `newcode/tools/shell.py` | cwd 缺省取 ctx cwd |
| 修改 | `newcode/agent/agent.py` | FileTracker 用 resolve_path（:607） |
| 新建 | `tests/test_tools_cwd.py` | 6 工具 ctx cwd 解析单测 |
| 修改 | `newcode/subagent/types.py` | AgentDefinition 加 isolation 字段 |
| 修改 | `newcode/subagent/parser.py` | 解析 isolation frontmatter |
| 修改 | `tests/test_subagent_parser.py` | 增加 isolation 用例（扩展既有） |
| 新建 | `newcode/tools/agent_worktree.py` | _execute_with_worktree（import build_worktree_notice） |
| 修改 | `newcode/tools/agent_tool.py` | 加 worktree_mgr 字段 + isolation 分支（强制前台） |
...
| 修改 | `newcode/slash/ui.py` | 加 WorktreeAccessor 协议 + WorktreeSummary |
| 新建 | `newcode/slash/commands/worktree.py` | /worktree handler + 子命令解析 |
| 修改 | `newcode/slash/commands/__init__.py` | 注册 worktree 模块 |
| 新建 | `tests/test_worktree_command.py` | /worktree 命令 handler 单测（stub UI/accessor） |
| 新建 | `newcode/tui/worktree_adapter.py` | 实现 WorktreeAccessor（包装 Manager + set_active_cwd） |
| 修改 | `newcode/tui/app.py` | REPL.worktree_mgr / active_cwd / run 注入 / get_cwd / worktree_accessor |
| 新建 | `tests/test_worktree_tui.py` | TUI active_cwd 注入单测 |
| 修改 | `newcode/main.py` | --resume / wm 装配 / sweep 任务 / finally 清理 |
| 修改 | `.gitignore` | 本开发仓库手动加 .newcode/worktrees/ + worktree_session.json |
...
**文件：** `newcode/worktree/types.py` + `newcode/worktree/slug.py` + `tests/test_worktree_slug.py`
**依赖：** 无
**步骤：**
1. 创建子包 `newcode/worktree/`，加 `__init__.py`（暂空导出，后续补）
...
**文件：** `newcode/worktree/session.py`
**依赖：** T1
**步骤：**
...
**文件：** `newcode/worktree/git.py` + `tests/test_worktree_git.py`
**依赖：** 无
**步骤：**
...
**文件：** `newcode/worktree/create.py` + `tests/test_worktree_create.py`
**依赖：** T4
**步骤：**
...
2. `_perform_post_creation_setup` 四子函数（各 try/except 仅警告）：
   - A `copy_local_configs`：复制 `.newcode/config.local.yaml`、`config.yaml`、`permissions*.yaml`、`agents/`、`skills/`（跳过 worktrees/sessions/memory/monitor；源缺失/目标存在跳过）
...
**文件：** `newcode/worktree/lifecycle.py` + `tests/test_worktree_lifecycle.py`
**依赖：** T5
**步骤：**
...
**文件：** `newcode/worktree/sweep.py` + `tests/test_worktree_sweep.py`
**依赖：** T6
**步骤：**
...
**文件：** `newcode/worktree/config.py` + `tests/test_worktree_config.py`
**依赖：** 无
**步骤：**
1. `WorktreesConfig` dataclass（enable/auto_cleanup/background_cleanup/cleanup_interval_minutes/expire_minutes）
2. `load_worktree_config(project_root)`：三层 `~/.newcode/config.yaml` → `<project>/.newcode/config.yaml` → `<project>/.newcode/config.local.yaml` 追加合并；非法值 warning 用缺省（镜像 `subagent/config.py`）
3. 测试：无配置全缺省；local 覆盖 project；非法值降级警告
**验证：** `pytest tests/test_worktree_config.py -v`

## T9: tool ctx

**文件：** `newcode/tools/cwd.py` + `tests/test_tool_ctx.py`
**依赖：** 无（与 T1-T8 并行）
**步骤：**
...
**文件：** `newcode/tools/{file_ops,search,shell}.py`、`newcode/agent/agent.py`、`tests/test_tools_cwd.py`
**依赖：** T9
**步骤：**
...
**文件：** `newcode/subagent/types.py`、`newcode/subagent/parser.py`、`tests/test_subagent_parser.py`
**依赖：** 无
**步骤：**
...
**文件：** `newcode/worktree/notice.py` + `newcode/tools/agent_worktree.py` + `tests/test_agent_worktree.py`
**依赖：** T6、T9、T11
**步骤：**
1. `worktree/notice.py`：`build_worktree_notice(parent_cwd, wt_path) -> str`（spec F8.3 模板，唯一实现处）
2. `agent_worktree.py` 顶部 `from newcode.worktree import Manager, random_agent_name` 与 `from newcode.worktree.notice import build_worktree_notice`（worktree 包不依赖 agent，无循环）
...
**文件：** `newcode/tools/agent_tool.py`、`tests/test_agent_tool.py`
**依赖：** T12
**步骤：**
...
**文件：** `newcode/slash/ui.py`、`newcode/slash/commands/worktree.py`、`newcode/slash/commands/__init__.py`、`tests/test_worktree_command.py`
**依赖：** T4
**步骤：**
...
**文件：** `newcode/tui/worktree_adapter.py`、`newcode/tui/app.py`、`tests/test_worktree_tui.py`
**依赖：** T12、T14
**步骤：**
...
**验证：** `pytest tests/test_worktree_tui.py -v`；`python -m newcode` 可启动
...
**验证：** `python -m newcode --version` 正常；全量测试通过
...
3. `ruff check newcode tests` 通过；`python -m newcode --version` 可启动
4. 确认 docs/ 未被改动（N14）
**验证：** 全部通过
...
**依赖：** T17
**步骤：**
1. 准备临时 git 仓库 + 项目级自定义角色 `.newcode/agents/worktree-writer.md`（`isolation: worktree`）
2. tmux 启动 `python -m newcode`，跑 checklist 端到端场景（见 checklist.md）
