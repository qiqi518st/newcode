# MewCode ch14 - Git Worktree 文件系统隔离 Checklist
...
- [ ] **AC2 / AC3 目录与分支命名**：`create("alice", ...)` → `.mewcode/worktrees/alice/` + 分支 `worktree-alice`；`create("team/alice", ...)` → `.mewcode/worktrees/team+alice/` + 分支 `worktree-team+alice`；`git branch` 输出可见（验证：`pytest tests/test_worktree_manager.py`）
...
- [ ] **AC5 / 配置复制**：主仓库存在 `.mewcode/config.local.yaml` → worktree 内同位置出现该文件（验证：`pytest tests/test_worktree_create.py`）
...
- [ ] **AC20 / session 持久化**：session 持久化到 `.mewcode/worktree_session.json`；worktree 目录被外部删除后启动清空 + stderr 警告（验证：`pytest tests/test_worktree_manager.py`）
...
- [ ] **AC24 / 非 git 优雅降级**：非 git 仓库 / 无 commit / git 缺失 → worktree 命令返回结构化错误或降级警告，主流程不崩（验证：`pytest tests/test_worktree_manager.py`；在非 git 目录 `python -m mewcode` 启动观察）

## 集成

- [ ] **WorktreeAccessor 协议解耦**：`mewcode/slash/` 不 import `mewcode/worktree`（协议定义在 slash/ui.py，适配器在 tui）（验证：grep 确认无反向 import）
- [ ] **slash 注册无冲突**：`register_all` 无命令名/别名冲突，`/worktree` 可注册（验证：`pytest` 启动注册路径；`python -m mewcode` 启动无冲突报错）
...
- [ ] **lint 通过**：`ruff check mewcode tests` 无错误；`ruff format --check` 通过（如有配置）
- [ ] **可启动**：`python -m mewcode --version` 正常；非 git 目录启动降级警告不崩（验证：运行）
...
- [ ] **场景 6（--resume）**：`/worktree enter x` 后退出，`python -m mewcode --resume` 重启 → session 恢复、`/worktree list` 可见、可 enter（验证：tmux 或 CLI）
- [ ] **场景 7（优雅降级）**：在非 git 目录运行 → `/worktree list`/`create` 返回结构化错误或「Worktree 功能未启用」，不崩（验证：tmux）
- [ ] **场景 8（tmux 实跑）**：临时 git 仓库 + 项目级角色 `.mewcode/agents/worktree-writer.md`（`isolation: worktree`）→ 触发子 Agent 改 `server.py` → 主目录 `server.py` 未变、worktree 副本已变；留盘/清理符合预期（验证：tmux 手动）
