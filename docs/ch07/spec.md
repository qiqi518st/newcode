...
- **复用权限**：MCP 工具天然走已有的「规则 → 模式兜底 → 人在回路」链路，默认按命令执行类每次确认，自报只读（`readOnlyHint`）的按只读类放行并可并发；本章为接入 MCP 对 permission 做一次性泛化（后续新增 MCP 工具不再需改 permission 源码）。
...
- **permission 包为接入 MCP 做一次性泛化**：规则解析正则放宽接受任意合法工具名（含 `mcp__` 前缀）、工具名支持 `*` 通配匹配、allow_always 对 MCP 工具落盘裸工具名精确规则。**泛化之后，未来新增 MCP 工具无需再改 permission 源码**即可被规则与模式引擎覆盖（即「permission 零改动」面向的是未来新增工具，而非 ch07 接入本身）。
...
- **N8: 代码规范**——`ruff check` / `ruff format --check` / `mypy`（可选 strict 子集）/ `pytest` 全过。
...
- **AC9: 调用超时与错误回灌**——`call_tool` 30s 超时或协议错误转为错误态的结构化错误结果回灌给模型，Agent Loop 不中断，可在后续轮调整。（F7/F10/N5）
...
- **AC15: 代码规范**——`ruff format --check .` 无 diff；`ruff check .` 无告警；`pytest`（含 `tests/test_mcp_*.py`）通过；`pytest tests/test_mcp_*.py` 无悬挂 task / 死锁（异步测试沿用 repo 现有 `@pytest.mark.anyio` 标记）。（N8）