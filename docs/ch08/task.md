# NewCode ch08 — 上下文管理 任务拆解 (task.md)
...
> docs 保护：本文件是 new-spec 流程产物（允许写入）；其余测试/验证过程不改动 docs/ 下任何已存在文件。

## 文件清单

| 路径 | 类型 | 职责 |
|------|------|------|
| `newcode/context/__init__.py` | 新建 | 门面导出 |
| `newcode/context/constants.py` | 新建 | 全部硬编码阈值常量 |
| `newcode/context/session.py` | 新建 | `SessionContext` / `SessionPaths` / `new_session_context` |
| `newcode/context/replacement.py` | 新建 | `ContentReplacementState`（`decide_once` 账本） |
| `newcode/context/autogate.py` | 新建 | `AutoCompactGate`（仅自动路径连续失败闸） |
| `newcode/context/files.py` | 新建 | `FileTracker` + `TrackedFile`（锁保护） |
| `newcode/context/tokens.py` | 新建 | 纯函数 `estimate_tokens` / `usage_to_anchor` / `estimate_messages` / `message_chars` |
| `newcode/context/window.py` | 新建 | `get_context_window_for_model` 四级解析 |
| `newcode/context/capabilities.py` | 新建 | 静态能力表 `CAPABILITIES` |
| `newcode/context/offload.py` | 新建 | `offload_and_snip` / `spill_single` / `build_preview` |
| `newcode/context/dropper.py` | 新建 | `MessageGroupDropper`（F27 丢消息组） |
| `newcode/context/recovery.py` | 新建 | `RecoveryBuilder` + `RecoveryBundle` + `BOUNDARY_NOTICE` |
| `newcode/context/summarize.py` | 新建 | `Summarizer` / `SummarizeConfig` / `CompactOutcome` / `pick_recent_tail` / `ptl_retry` |
| `newcode/context/manager.py` | 新建 | `ContextManager`（manage_context / compact_now / force_compact / update_anchor） |
| `newcode/context/skill.py` | 新建 | `Skill` + `SkillRegistry`（骨架，TODO） |
| `scripts/probe_context_window.py` | 新建 | 独立探测脚本（不在主流程，手工回填能力表） |
| `newcode/conversation/manager.py` | 修改 | `get_messages_ref` / `replace_history` / `_trim` 不拆对降级 |
| `newcode/agent/events.py` | 修改 | `CONTEXT_COMPACTING` / `COMPACT_FAILED` 事件 |
| `newcode/agent/agent.py` | 修改 | `_run_lock` 贯穿 / manage_context 钩子 / 文件追踪回填 / PTL 兜底 / run_force_compact / update_anchor / emit 事件 |
| `newcode/llm/__init__.py` | 修改 | 新增 `PromptTooLongError` 哨兵异常 |
| `newcode/prompt/assembler.py` | 修改 | `PromptPayload.max_output_tokens` 字段 |
| `newcode/provider/anthropic.py` | 修改 | PTL 识别（`PromptTooLongError`）/ max_output_tokens 透传 |
| `newcode/provider/openai.py` | 修改 | PTL 识别（`PromptTooLongError`） |
| `newcode/tui/app.py` | 修改 | `BUILTIN_COMMANDS` 注册表 / `/compact` / 压缩 UX / 熔断菜单 |
| `newcode/main.py` | 修改 | 构造 `ContextManager` 注入 Agent |
| `newcode/__init__.py` | 修改 | `__version__` 升 `0.8.0` |
| `pyproject.toml` | 修改 | `version` 升 `0.8.0` |
| `.gitignore` | 修改 | 追加 `.newcode/sessions/` |
...
- **文件**：`newcode/context/__init__.py`（空门面）、`newcode/context/constants.py`
- **依赖**：无
- **步骤**：
  1. 新建 `newcode/context/` 目录，放空 `__init__.py`（后续 T18 按导出需求填充）。
...
- **验证**：`python -c "from newcode.context import constants; print(constants.SINGLE_RESULT_THRESHOLD)"` → `50000`；`ruff check newcode/context/constants.py` 无告警。

---

## T2 - SessionContext / SessionPaths / new_session_context

- **文件**：`newcode/context/session.py`
- **依赖**：T1
- **步骤**：
  1. 定义 `@dataclass SessionContext(session_id: str, spill_dir: str)`。
  2. `new_session_context(workspace: str) -> SessionContext`：`session_id = f"{int(time.time())}-{secrets.token_hex(4)}"`（`secrets` 失败极端情况 `try/except` 退 `random` + `logging.warning`）；`spill_dir = workspace/.newcode/sessions/<sid>/tool-results`；`Path(spill_dir).mkdir(parents=True, exist_ok=True)`（已存在不报错）。
...
- **验证**：`python -c "from newcode.context.session import new_session_context; print(new_session_context('.'))"` 输出合理且目录存在；`ruff check newcode/context/session.py` 无告警。

---

## T3 - ContentReplacementState（decide_once 账本）

- **文件**：`newcode/context/replacement.py`
- **依赖**：T2
- **步骤**：
...
- **验证**：`ruff check newcode/context/replacement.py` 无告警；目测 `decide_once` 是唯一临界区入口、`decision_for` 只读。

---

## T4 - AutoCompactGate（自动路径连续失败闸）

- **文件**：`newcode/context/autogate.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`ruff check newcode/context/autogate.py` 无告警；目测无手动/紧急接口。

---

## T5 - FileTracker + TrackedFile（文件追踪）

- **文件**：`newcode/context/files.py`
- **依赖**：无
- **步骤**：
...
- **验证**：`ruff check newcode/context/files.py` 无告警；目测 `recent` 返回拷贝不暴露内部 dict。

---

## T6 - tokens 纯函数估算

- **文件**：`newcode/context/tokens.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`python -c "from newcode.context.tokens import estimate_tokens; print(estimate_tokens(0, [], 0))"` → `0`；`ruff check newcode/context/tokens.py` 无告警。

---

## T7 - Context Window 四级解析 + 能力表

- **文件**：`newcode/context/window.py`、`newcode/context/capabilities.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`python -c "from newcode.context.window import get_context_window_for_model; print(get_context_window_for_model('claude-sonnet-4','anthropic'))"` → `200000`；`ruff check newcode/context/window.py newcode/context/capabilities.py` 无告警。

---

## T8 - 第一层：spill_single + build_preview

- **文件**：`newcode/context/offload.py`
- **依赖**：T2
- **步骤**：
...
- **验证**：`python -c "from newcode.context.offload import build_preview; a=build_preview(60000,'x'*100,'/tmp/f'); assert a==build_preview(60000,'x'*100,'/tmp/f'); print('stable')"` → `stable`；`ruff check newcode/context/offload.py` 无告警。
...
- **步骤**：
  1. `async def offload_and_snip(messages: list[Message], state: ContentReplacementState, session_paths: SessionPaths) -> int`（返回被替换的项数）：
     ...
     - 返回被替换的项数。
- **验证**：`ruff check newcode/context/offload.py` 无告警；目测每个 candidate 只经 `decide_once` 走一次。

---

## T10 - MessageGroupDropper（F27 丢消息组）

- **文件**：`newcode/context/dropper.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`ruff check newcode/context/dropper.py` 无告警；目测分组不拆 tool_use/tool_result 对。

---

## T11 - RecoveryBuilder（恢复段三块）

- **文件**：`newcode/context/recovery.py`
- **依赖**：T5、T6
- **步骤**：
...
- **验证**：`ruff check newcode/context/recovery.py` 无告警；目测 `build` 只读 `tool_defs` 引用不重算。

---

## T12 - 摘要 prompt 构造与解析

- **文件**：`newcode/context/summarize.py`
- **依赖**：T1
- **步骤**：
...
- **验证**：`ruff check newcode/context/summarize.py` 无告警；目测 `serialize_conversation` 确定性输出。
...
- **步骤**：
  ...
  2. `_join_after_summary(summary_and_recovery: Message, recent: list[Message]) -> list[Message]`：
     ...
     - 否则正常拼接。
- **验证**：`ruff check newcode/context/summarize.py` 无告警；目测 `_join_after_summary` 保证无连续 user。
...
- **验证**：`ruff check newcode/context/summarize.py` 无告警；目测摘要请求不更新锚点（`summarize` 内不调 `update_anchor`）。
...
- **验证**：`ruff check newcode/context/manager.py` 无告警；目测三路径收尾（自动停触发+报错 / 手动显示失败 / 紧急报错且原请求不重试）。

---

## T16 - Skill 骨架

- **文件**：`newcode/context/skill.py`
- **依赖**：无
- **步骤**：
...
- **验证**：`ruff check newcode/context/skill.py` 无告警；目测 registry 空时注入分支跳过。
...
- **验证**：`ruff check newcode/conversation/manager.py` 无告警；目测 `_trim` 后 tool 结果不落单。

---

## T18 - 子包门面

- **文件**：`newcode/context/__init__.py`
- **依赖**：T1–T16 全部
- **步骤**：
...
- **验证**：`python -c "from newcode.context import ContextManager, get_context_window_for_model, estimate_tokens, ContentReplacementState, FileTracker, Skill, SkillRegistry; print('ok')"` → `ok`；`ruff check newcode/context/__init__.py` 无告警。

---

## T19 - PromptTooLongError 哨兵 + provider PTL 包装 + max_output_tokens

- **文件**：`newcode/llm/__init__.py`、`newcode/prompt/assembler.py`、`newcode/provider/anthropic.py`、`newcode/provider/openai.py`
- **依赖**：无（与 T1–T18 并行）
- **步骤**：
...
- **验证**：`python -c "from newcode.llm import PromptTooLongError; print('ok')"` → `ok`；`ruff check newcode/llm/__init__.py newcode/prompt/assembler.py newcode/provider/anthropic.py newcode/provider/openai.py` 无告警。
...
- **验证**：`ruff check newcode/agent/events.py newcode/agent/agent.py` 无告警。
...
- **验证**：`ruff check newcode/agent/agent.py` 无告警；目测只对 read_file 成功结果触发。
...
- **验证**：`ruff check newcode/agent/agent.py` 无告警；目测紧急路径不二次 ForceCompact（F26）。
...
- **验证**：`ruff check newcode/tui/app.py` 无告警；目测命令路径不写 conversation。
...
- **验证**：`python -m newcode --version` → `newcode 0.8.0`；`python -c "import newcode.main; print('import ok')"`；`ruff check newcode/main.py` 无告警。
...
- **依赖**：无
- **步骤**：
  1. `python scripts/probe_context_window.py --protocol anthropic --model <name> [--base-url …] [--api-key …]`：二分逼近构造逐步增长的填充 prompt 发请求直到 provider 返回 PTL，二分定位边界；stdout 打印结果 + 「经验下界，手工填入 `newcode/context/capabilities.py`」提示。
  2. **不在 Agent 主流程、不被 import、不被 F29 引用**（F30）。
- **验证**：`python scripts/probe_context_window.py --help` → 正常打印用法退出 0（无需 API key）；`grep -rn "probe_context_window" newcode/` 无命中。真探测标「待人工验证」（需 API key + 网络，结果手工回填能力表并提交）。

---

## T26 - 版本号与 .gitignore

- **文件**：`newcode/__init__.py`、`pyproject.toml`、`.gitignore`
- **依赖**：无
- **步骤**：
  1. `__version__` 从 `"0.7.0"` 改 `"0.8.0"`（两处：`__init__.py` + `pyproject.toml`）。
  2. `.gitignore` 追加 `.newcode/sessions/`。
- **验证**：`python -c "import newcode; print(newcode.__version__)"` → `0.8.0`；`grep -n '0.7.0' pyproject.toml newcode/__init__.py` 无命中。
...
- **依赖**：T23、T24、T25
- **步骤**：
  1. `pip install -e .[dev]`；tmux 启动 `python -m newcode`，配置好 provider。
  2. 触发一次大文件 ReadFile（构造 80KB 文件）→ `.newcode/sessions/<id>/tool-results/` 出现该工具调用 id 的文件；下一轮请求该工具结果展示为预览体（AC1 端侧）。
...
- **步骤**：
  ...
  4. `.gitignore` 确认含 `.newcode/sessions/`；`git status` 确认 sessions 不入库。
  5. **docs 保护自检**：`git status docs/` 仅显示 `docs/ch08/` 新增（四份 new-spec 流程文档），`git diff docs/` 对既有文档无修改。
- **验证：** 全部通过。
