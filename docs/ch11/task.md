# NewCode ch11 - Skill 技能包系统 Tasks

> 顺序执行。每完成一个任务跑 `ruff check newcode/skills newcode/tools/load_skill.py` 确保无 lint 错；接入主流程的任务（T21）做完后立刻跑一次端到端冒烟（T22）再进下一项。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `newcode/__init__.py`、`pyproject.toml` | 版本号 0.10.0 → 0.11.0 |
| 新建 | `newcode/skills/__init__.py` | 包导出（Catalog/ActiveSkills/Executor/SkillSource） |
| 新建 | `newcode/skills/constants.py` | 预算/路径常量、SYSTEM_TOOL_NAMES |
| 新建 | `newcode/skills/types.py` | SkillMeta/Skill/SkillSource/ActiveEntry/ToolSchema |
| 新建 | `newcode/skills/parser.py` | 解析 frontmatter+body、归一化、校验 |
| 新建 | `newcode/skills/render.py` | render_body（$ARGUMENTS/兜底/提示） |
| 新建 | `newcode/skills/catalog.py` | 三级扫描/覆盖/热加载/validate_tools/disabled/get_catalog |
| 新建 | `newcode/skills/active.py` | ActiveSkills（激活/失活/预算淘汰） |
| 新建 | `newcode/skills/adapter.py` | catalog_to_prompt_items / active_to_prompt_entries |
| 新建 | `newcode/skills/executor.py` | inline / fork 执行器 |
| 新建 | `newcode/skills/script_tool.py` | 目录型工具子进程壳 |
| 新建 | `newcode/skills/builtin/{commit,review,test}.md` | 三个内置 Skill |
| 新建 | `newcode/prompt/skills_block.py` | env 段 Skill 渲染 + 瘦类型 |
| 新建 | `newcode/tools/load_skill.py` | LoadSkill 系统工具 |
| 新建 | `newcode/slash/commands/skill.py` | /skill 管理命令 |
| 新建 | `newcode/slash/commands/skill_register.py` | Skill 动态注册为 /名字 |
| 修改 | `newcode/tools/base.py` | Tool 协议加 is_system |
| 修改 | `newcode/tools/registry.py` | system_definitions / definitions_filtered |
| 修改 | `newcode/agent/agent.py` | with_catalog / activate_skill / env 合成 |
| 修改 | `newcode/context/recovery.py`、`context/manager.py` | 预算淘汰 + 恢复段注入 |
| 修改 | `newcode/slash/registry.py` | unregister / remove_by |
| 修改 | `newcode/slash/ui.py` | 4 个 UI 方法 |
| 修改 | `newcode/slash/commands/clear.py` | 追加 clear_active_skills |
| 修改 | `newcode/slash/commands/__init__.py` | 移除 review、加 skill/skill_register |
| 修改 | `newcode/main.py` | 装配 Skill 系统 |
| 删除 | `newcode/slash/commands/review.py` | 被 review Skill 接管 |
| 删除 | `newcode/context/skill.py` | 骨架被 skills/ 取代 |
...
**依赖：** 无
**完成标准：**
- [ ] `newcode/__init__.py` 的 `__version__` = `"0.11.0"`
- [ ] `pyproject.toml` 的 `version` = `"0.11.0"`，两处一致
- [ ] 独立提交 `chore: bump version to 0.11.0`

**验证：** `export PYTHONIOENCODING=utf-8 && python -c "import newcode; print(newcode.__version__)"` 输出 0.11.0。

## T2: skills 包骨架——constants + types

**文件：** `newcode/skills/__init__.py`、`newcode/skills/constants.py`、`newcode/skills/types.py`
**依赖：** T1
**完成标准：**
- [ ] `constants.py`：`ACTIVE_SKILL_TOKEN_BUDGET = 4_000`、`RECENT_DEFAULT_N = 5`、`PROJECT_SKILLS_DIR = ".newcode/skills"`、`USER_SKILLS_DIR = "~/.newcode/skills"`、`DISABLED_STATE_FILE = "~/.newcode/skills/disabled.json"`、内置目录路径
...
**验证：** `python -c "from newcode.skills.types import SkillMeta, Skill, SkillSource; s=SkillMeta(name='commit'); assert s.mode=='inline' and s.fork_context=='none'"`。

## T3: skills/parser.py

**文件：** `newcode/skills/parser.py`
**依赖：** T2
**完成标准：**
...
**验证：** `python -c "from newcode.skills.parser import normalize_name; assert normalize_name('My_Skill')=='my-skill'"`；构造非法 frontmatter smoke。

## T4: skills/render.py

**文件：** `newcode/skills/render.py`
**依赖：** T2
**完成标准：**
...
- [ ] disabled 集合：`is_disabled(name)` / `set_disabled(name, bool)`，落盘 `~/.newcode/skills/disabled.json`（F7.8）
...
**文件：** `newcode/skills/adapter.py`、`newcode/prompt/skills_block.py`
**依赖：** T2、T7
**完成标准：**
...
**验证：** `python -c "from newcode.prompt.skills_block import render_active_skills_block; assert render_active_skills_block([])==''"`；构造 entries 断言输出含 name/body。

## T9: tools/base.py + registry.py 扩展

**文件：** `newcode/tools/base.py`、`newcode/tools/registry.py`
**依赖：** 无（独立）
**完成标准：**
...
**文件：** `newcode/tools/load_skill.py`
**依赖：** T5、T7、T9
**完成标准：**
...
**文件：** `newcode/skills/script_tool.py`
**依赖：** T2、T9
**完成标准：**
...
**文件：** `newcode/skills/builtin/commit.md`、`review.md`、`test.md`
**依赖：** T2（格式）
**完成标准：**
...
**文件：** `newcode/context/recovery.py`、`newcode/context/manager.py`、删除 `newcode/context/skill.py`
**依赖：** T7
**完成标准：**
...
**文件：** `newcode/slash/commands/skill_register.py`
**依赖：** T12、T13、T17、T18
**完成标准：**
...
**文件：** `newcode/slash/commands/skill.py`（新）、`clear.py`（改）、`__init__.py`（改）、删除 `review.py`
**依赖：** T17、T18、T19
**完成标准：**
...
**验证：** `python -c "import newcode.main"` 通过；T22 端到端冒烟覆盖装配后行为。

## T22: 手动端到端冒烟（参考模板 T10 五步）

**文件：** 无（仅运行验证）；测试用 skill 目录 `.newcode/skills/test-skill/SKILL.md`
**依赖：** T21
**完成标准：**
- [ ] 创建测试 skill：`.newcode/skills/test-skill/SKILL.md`（frontmatter `name: test-skill / description: A test skill / mode: inline`，body `Echo hello`）
