# MewCode ch10 - SlashCommand 框架 Plan
...
核心思路：**命令框架（mewcode/slash/）与 UI 完全解耦**。TUI 只负责把回车输入交给分流器、把 UIController 实现注入；命令注册中心持有元数据与 handler，handler 通过 UIController 抽象操作界面，不 import prompt_toolkit / rich。

## 组件划分

| 模块 | 文件 | 职责 |
|---|---|---|
| 命令注册中心 | mewcode/slash/registry.py | CommandKind、CommandDef、CommandRegistry（RLock、启动期冲突检测、lookup/list/complete） |
| 命令解析器 | mewcode/slash/parser.py | parse_command(text) → (name, args) 或 None；大小写不敏感 |
| 命令上下文 | mewcode/slash/context.py | CommandContext 打包全部依赖 |
| UI 抽象 | mewcode/slash/ui.py | UIController 抽象接口（Protocol/ABC） |
| 内置命令 | mewcode/slash/commands/*.py | 各命令实现（见文件组织） |
| 命令装配 | mewcode/slash/commands/__init__.py | register_all(registry) |
| TUI 接入 | mewcode/tui/app.py | 分流器、Tab 补全 completer、状态栏、RichUIController 实现 |
| 装配 | mewcode/main.py | 构造 registry、register_all、组装 CommandContext、注入 REPL |
| 权限 API | mewcode/permission/checker.py | 新增 count_rules / add_rule / reset_rules（/permission_* 需要） |
| 会话 API | mewcode/session/runtime.py（复用） | /session_* 复用现有 SessionRuntime / SessionArchive |
| 记忆 API | mewcode/memory/store.py（复用） | /memory_* 复用现有 MemoryStore.list_notes / apply / clear |
...
### mewcode/permission/checker.py 新增
...
mewcode/context/manager.py      # 修改：新增 reset_for_new_session()（清 L1 替换账本 + 自动闸 + 锚点，/clear 用）
mewcode/permission/checker.py   # 修改：count_rules / add_rule / reset_rules
mewcode/tui/app.py              # 修改：分流器 / completer / 状态栏 / RichUIController
mewcode/main.py                 # 修改：装配接线 + 启动冲突检测 panic
...
| 决策点 | 选择 | 理由 |
|---|---|---|
| 包位置 | 独立 mewcode/slash/ | Skill 动态注册复用、跨 UI 解耦（用户拍板） |
