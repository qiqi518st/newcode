...
本项目的 Bash 环境（Git Bash）中文输出会乱码。每次运行 Python 命令时，必须设置 UTF-8 编码：

```bash
export PYTHONIOENCODING=utf-8 && python ...
```

或完整版：
```bash
export PYTHONIOENCODING=utf-8 && export LC_ALL=C.UTF-8 && python ...
```
...
- **接线测试必须自动跑，不得依赖真实终端。** 交互功能（TUI、API）用 mock 驱动真实代码路径，断言行为。测试要能在无真实终端 / 无 API key 的环境下执行（如用 `object.__new__` 绕过 PromptSession、用 mock provider 产出事件）。
- **mock 必须驱动真实代码路径，不是验证库本身。** 「Rich Live 能渲染 Markdown」验证的是库；「REPL._consume_stream 调用了渲染」验证的是我们的代码。两者都测，后者才是防回归的关键。
- **重复路径必须交叉验证。** 同一渲染逻辑分散在多条路径（如 TUI 与单次调用）时，只测其中一条会让另一条悄悄坏掉。每条路径都要有对应测试。
- **每个测试标注它防的 bug。** 在测试文件 docstring 里写清背景（如「曾出现 _consume_stream 只累积文本不渲染」），防止后人误删关键用例。
...
- **验证受阻必须上报，不得静默跳过。** 当某项验收因环境限制（无 API key、无真实终端、无权限、无法交互等）无法执行时，必须：
  - 说明**为什么**无法验证（具体的环境限制）
  - 评估**跳过风险**（该验收可能掩盖什么 bug）
  - 提出**替代验证**（mock / 单测覆盖哪部分逻辑，真实环境由谁何时补验）
  - 在报告中单独列为「待人工验证」，**不得混入「通过」**
  - 「无法验证」≠「通过」。宁可停下问，不可静默跳过。
...
### 待人工验证（如有）
- [ ] 条目 4 — 原因：环境限制（说明），替代验证：...，风险：...，补验：由谁何时
...
| 「应该没问题了」 | 跑一下。先有证据再下结论 |
| 「这个场景没法测，先标通过吧」 | 无法验证 ≠ 通过。标「待人工验证」，说清原因和风险 |
...
| 章节 | 版本号 | 说明 |
|------|--------|------|
| ch01（LLM API 打通） | 0.1.0 | 能发起请求并接收流式回复 |
| ch02（TUI 多轮对话） | 0.2.0 | 终端交互、滑动窗口、双 Provider |
| ch03（工具系统） | 0.3.0 | 六个核心工具、单轮闭环、Agent 编排 |
| ch04（Agent Loop） | 0.4.0 | 多工具连环调用、自动循环（规划中） |
